...
if ! grep -qf release/tag s3.quarks-operator/version; then
  echo -n "Operator binary version does not match Github tag from Github release: "
  cat s3.quarks-operator/version
  exit 1
fi

# Docker versions are not strictly following the semantic versioning spc
docker_tag=$(tar xOfz s3.helm-charts/quarks-*.tgz quarks/values.yaml | grep "  tag: v")
...
echo "zip quarks binary"
cp s3.quarks-operator/quarks-operator-* out
gzip out/quarks-operator-*
...
binary_version=$(cat s3.quarks-operator/version)
...
helm_chart_url="release/helm-charts/quarks-$version.tgz"
...
* https://s3.amazonaws.com/cf-operators/release/binaries/quarks-operator-$binary_version
